...
- Renova e persiste os tokens do Mercado Livre. `/relatorio` disponível a todos os membros do servidor configurado. `/conectar` e `/status` restritos ao seu ID do Discord.
...
- **Comando restrito:** `/conectar` e `/status` exigem seu `DISCORD_OWNER_ID`. `/relatorio` aceita todos os membros do servidor configurado; confira também permissões de comandos do Discord.
...
## Atualização: relatório para todos

Substitua `main.py` no repositório pelo arquivo desta versão e faça Commit changes. Aguarde o novo deploy do Railway. Não precisa recriar o bot ou reautorizar o Mercado Livre se o volume e as variáveis foram mantidos. Todos os membros do servidor configurado podem usar `/relatorio`; a resposta permanece visível somente a quem pediu. Os relatórios automáticos continuam no canal configurado. Permissões adicionais de comandos no próprio Discord ainda se aplicam.
